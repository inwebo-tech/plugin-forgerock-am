...
public class InWeboAuth extends AMLoginModule {
  ...
  private int authenticateExtendedValidator(final Callback[] callbacks) throws InvalidPasswordException {
    ...
    final JSONObject result = inWeboRestAuthenticator.authenticate(sharedUserName, password);
    if (result != null && RESULT_CODE_OK.equals(result.get("err"))) {
      if (DEBUG.messageEnabled()) {
        DEBUG.message("InWeboAuthModule::auth - Login by '{}' succeeded", sharedUserName);
      }
      return LOGIN_SUCCEED;
    } else {
      if (DEBUG.errorEnabled()) {
        DEBUG.error("InWeboAuthModule::auth - Login by '{}' failed with result '{}'", sharedUserName, result);
      }
